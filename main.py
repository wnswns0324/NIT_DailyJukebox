...
def post_instagram(api, image_path, track_name, artist_names, album_name, i):
  ...
  api.photo_upload(
      image_path, f"{now.year}.{now.month}.{now.day} Today's {thistime} #{genre[i]}\n{message}")
  time.sleep(5)
...
def download():
  ...
  for i in range(3):
    ...
    for idx, track in enumerate(tracks):
      ...
      artist_names[i].append(', '.join([artist['name']
...
def upload(api):
  ...
  for i in range(3):
    ...
    post_instagram(api, image_path, selected_track_name,
                   selected_artist_names, selected_album_name, i)
  count += 1
  dir = db.reference('')
